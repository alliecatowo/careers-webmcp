...
Evidence: `src/webmcp/presence/AgentPresenceLayer.tsx` returns `null` until `agentPresent` flips, which only `beginActivity` (called from the tool wrapper) can do. `tests/e2e/presence.spec.ts` asserts that with the shim installed and all 16 tools registered, no presence DOM exists until a tool is actually invoked. There is no chat panel, no prompt box, no way to start an agent, and nothing persists.
